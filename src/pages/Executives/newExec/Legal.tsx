...
const Legal: React.FC<IactionProps> = ({ next, prev, data, setData, execDocID, sugDocId, recDocId }) => {
  ...
  const { data: legalRegulatoryData, isLoading: incomingData } = useQuery(["legal", "legalRegulatory", requestId], () =>
    getExecData("legalRegulatory", requestId),
  );
  ...
  useEffect(() => {
    if (
      ...
      console.log(legalRegulatoryData?.data?.legalRegulatory);
    }
  ...
  const { mutate: postLegal } = useMutation(updateExec, {
    onError: (error) => {
      console.log(error);
