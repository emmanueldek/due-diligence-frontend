...
const Credit: React.FC<IactionProps> = ({ next, prev, data, setData, execDocID, sugDocId, recDocId }) => {
  ...
  const { data: creditData, isLoading: incomingData } = useQuery(["creditHistory", "creditHistory", requestId], () =>
    getExecData("creditHistory", requestId),
  );
  ...
  const { mutate: postCredit } = useMutation(updateExec, {
    onError: (error) => {
      console.log(error);
      Toast.error("document not saved");
    },
    onSuccess: () => {
      Toast.success("Saved documents successfully");
      if (newNext) {
        next();
      }
    },
  ...
  return (
    <div>
      <p className="font-[700] text-2xl">Credit</p>
