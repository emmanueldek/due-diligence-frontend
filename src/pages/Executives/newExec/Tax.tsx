...
import { IDataProps, ITaxProps } from "@/interface/userCreation";
...
const Tax: React.FC<IactionProps> = ({ next, prev, data, setData, execDocID, sugDocId, recDocId }) => {
  const { id } = useParams();
  const requestId = id || "";
  const [dataTab, setDataTab] = useState<number | null>(null);
  const [newNext, setNext] = useState(false);
  const [file, setFile] = useState<string>("");
  const [dataList, setDataList] = useState<IOrganisationProps[]>(data.tax || []);
  const [check, setCheck] = useState<number | null>(null);
  const [open, setOpen] = useState<number | null>(null);
  const taxData = { tax: dataList };
  const [checkAdd, setCheckAdd] = useState(false);

  const { data: taxComplianceData, isLoading: incomingData } = useQuery(
    ["taxCompliance", "texCompliance", requestId],
    () => getExecData("taxCompliance", requestId),
  );

  useEffect(() => {
    if (taxComplianceData?.data?.taxCompliance && Object.keys(taxComplianceData.data.taxCompliance).length > 0) {
      setDataList(taxComplianceData?.data?.taxCompliance);
    }
  ...
  const { mutate: postTax } = useMutation(updateExec, {
    onError: (error) => {
      console.log(error);
      Toast.error("Tax compliance not saved");
    },
    onSuccess: () => {
      Toast.success("Saved tax compliance successfully");
      if (newNext) {
        next();
      }
    },
  ...
  const handleQuerySubmit = () => {
    const data = { taxCompliance: dataList };
    if (dataList.length !== 0) {
      if (sugDocId) {
        const payLoad = { flag: "taxCompliance", data: data, orgId: sugDocId };
        acceptSuggest(payLoad);
      } else if (recDocId) {
        const payLoad = { flag: "taxCompliance", data: data, orgId: recDocId };
  ...
  return (
    <div>
      <p className="font-[700] text-2xl">Tax Compliance</p>
      ...
      <form action="" onSubmit={handleSubmit} className="my-5 space-y-5">
        ...
        {open === 2 && (
          <SavePublish data={taxData} onClose={handleClose} next={next} setData={setData} execDocID={execDocID} />
        )}
      </form>
    </div>
  );
