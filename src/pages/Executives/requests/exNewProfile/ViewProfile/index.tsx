...
import { acceptRequest, getSingleExecRequest } from "@/services/executiveService";
...
const ExecProfileRequest: React.FC = () => {
  ...
  const { data, isLoading } = useQuery(["singleRequestProfile"], () => getSingleExecRequest(requestId));
      ...
      <Wrapper className="bg-white px-10 py-10">
        {isLoading ? (
          ...
          <>
            ...
            <div>
              <p className="font-[700] text-xl my-5">New Executives Request</p>
            </div>
            ...
            <Wrapper className="px-5 shadow-md mt-10">
              <div className="w-[70%]">
                ...
                <div>
                  ...
                  <p className="text-xl font-[300] pt-2 text-justify">{singleData?.executiveDesc}</p>
                </div>
                <hr className="my-5 border-grey-100 opacity-50" />
                <p className="font-[700] text-[lg]">Supporting Document</p>
                <Link to={`${singleData?.executiveDoc}`}>
                  ...
                  <h1 className="text-xs mt-1 text-green-700">{singleData?.executiveDoc?.slice(81)}</h1>
                </Link>
              ...
              <div className="pt-5 space-x-2 flex justify-end">
                ...
                <button
                  ...
                  onClick={handleClick}
                >
                  <>{loadRequest ? <ClipLoader size={20} color={"#ffffff"} /> : <span>Create Executive</span>}</>
                </button>
              </div>
            </Wrapper>
          </>
...
export default ExecProfileRequest;
