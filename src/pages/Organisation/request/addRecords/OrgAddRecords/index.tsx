...
const RecordViewOrg: React.FC = () => {
  const navigate = useNavigate();

  const [open, setOpen] = useState<number | null | boolean>(null);
  const [singleData, setSingleData] = useState<IMoreDataProps>();
  const { id } = useParams();
  const requestId = id || " ";
  const recDocId = singleData?.theRequest?.recDocId;
  const orgId = singleData?.theRequest?.organizationId;

  const { data, isLoading } = useQuery(["singleOrgRecord"], () => getSingleRequest(requestId));
  ...
  const handleClick = () => {
    navigate(`/organisation/create/${orgId}`, { state: { rec: recDocId } });
  };
    ...
    <div>
            ...
            <div className="flex items-center space-x-5">
              {singleData?.theRequest?.avatar ? (
                ...
                <div className="rounded-lg overflow-hidden w-[72px] h-[72px] mt-3 text-[700] flex justify-center items-center bg-[#f59e0b] text-4xl text-white">
                  {singleData?.theRequest?.organizationName?.charAt(0)}
                </div>
              )}
              <div>
                <p className="font-[700] text-2xl">{singleData?.theRequest?.requestedBy}</p>
                <p>
                  {singleData?.theRequest?.requesterPosition} {singleData?.theRequest?.requesterOrganizationName}
                </p>
              </div>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-5">
                ...
                <div>
                  <p className="font-[700] text-2xl">{singleData?.theRequest?.organizationName}</p>
                  <p>
                    {singleData?.industry} {singleData?.location}
                  </p>
                </div>
              </div>
              {/* <Link
                to={`/organisation/view/${singleData?.theRequest?.organizationId}`}
                className="text-[#2F80ED] font-[600]"
              >
                View Profile
              </Link> */}
            </div>
            ...
            <div>
              ...
              <p className="text-xl font-[300] pt-2 text-justify">{singleData?.theRequest?.recordSection}</p>
            </div>
            <hr className="my-5 border-grey-100 opacity-50" />
            <div>
              <p className="font-[700] text-[lg]">Requested record</p>
              <p className="text-xl font-[300] pt-2 text-justify">{singleData?.theRequest?.recordInput}</p>
            </div>
          </div>
          <hr className="my-5 border-grey-100 opacity-50" />
      ...
      {open === 1 && (
        <RejectModal
          ...
          onClose={handleClose}
          buttonTitle="Decline"
          requestId={singleData?.theRequest?.recDocId || ""}
          flag="add_record"
        />
      )}
    </div>
