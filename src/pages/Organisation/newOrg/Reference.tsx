...
const Reference: React.FC<IactionProps> = ({ prev, data, setData, next, execDocID, sugDocId, recDocId }) => {
  ...
  const { data: refData, isLoading: incomingData } = useQuery(["refData", "referencesReputation", requestId], () =>
    getOrgData("referencesReputation", requestId),
  );
  ...
  return (
    <div>
      ...
      <form action="" onSubmit={handleSubmit} className="my-5 space-y-5">
        ...
        {open === 1 && <SaveDraftModal data={referenceData} onClose={handleClose} setData={setData} next={next} />}
        {open === 2 && (
          <SavePublish data={referenceData} onClose={handleClose} setData={setData} next={next} execDocID={execDocID} />
        )}
      </form>
    </div>
  );
